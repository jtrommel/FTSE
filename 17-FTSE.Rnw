...
ggplot(data = Fcl, aes(x =date)) +
    ...
    scale_x_date(limits = c(ymd("2019-01-01"), ymd("2019-05-24"))) +
    scale_y_continuous(limits = c(6000, 8000)) +
    JT.theme
@
  
\begin{figure}
\includegraphics[width=1\textwidth]{FTSE-startgrafiek}
\caption{}
...
N <- 36 # working days between start and end
Fcl <- grabFTSE("2019-04-09")
# limit the time series to 2019
Fcl %>% dplyr::filter(date > ymd("2018-12-31")) -> Fcl19
Fcl19$i <- Fcl19$i -Fcl19$i[1] + 1
n <- nrow(Fcl19)
...
\begin{figure}
\includegraphics[width=1\textwidth]{FTSE-forecast1}
\caption{}
\label{fig:forecast1}
\setfloatalignment{b}
\end{figure}
